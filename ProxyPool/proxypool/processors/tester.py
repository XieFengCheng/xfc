...
from proxypool.setting import TEST_TIMEOUT, TEST_BATCH, \
    TEST_URL, TEST_VALID_STATUS, TEST_ANONYMOUS, REDIS_KEY
    ...
    async def test(self, proxy: Proxy):
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
            try:
                ...
                async with session.get(TEST_URL, proxy=f'http://{proxy.string()}', timeout=TEST_TIMEOUT, allow_redirects=False) as response:
                    ...
                    if response.status == 200:
                        ...
                        self.redis.db.zrem(REDIS_KEY, proxy.string())
                        # logger.debug(f'proxy {proxy.string()} is invalid, decrease score')
                        logger.debug(f'proxy {proxy.string()} is invalid, remove score')
            except EXCEPTIONS:
                # self.redis.decrease(proxy)
                self.redis.db.zrem(REDIS_KEY, proxy.string())
                # logger.debug(f'proxy {proxy.string()} is invalid, decrease score')
