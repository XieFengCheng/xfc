...
class HuXiu(object):
    ...
    def company_list(self):
        ...
        keys = conn.rpop('keywords')
        # keys = conn.lindex('keywords', 0)
        print(keys)
        try:
            key = parse.quote(keys)
            keys_url = self.company_list_url.format(key)
            self.driver.get(keys_url)
            now_page = self.driver.current_url
            if 'captcha' not in str(now_page):
                et_late = etree.HTML(self.driver.page_source)
                page_total = ''.join([i.xpath('./li/a/text()')[-2] for i in et_late.xpath(
                    '//div[@class="result-footer"]/div[@class=" search-pager"]/ul')]).replace('.', '')
                for pg in range(int(page_total)):
                    pg_key_url = keys_url.replace('p0', 'p{}'.format(str(pg)))
                    conn.lpush('keys_pg_urls', pg_key_url)


            elif 'login' in str(now_page):
                print('## 返回登录页面 ##')
                conn.lpush('keywords', keys)
                self.visit_index()

            elif now_page[8:17] == 'antirobot':
                print('!! 验证点击验证码 !!')
                print(now_page)
                self.run_verify()
                et_late = etree.HTML(self.driver.page_source)
    ...
    def visit_index(self):
        ...
        if self.login_user and self.login_password:
            ...
            pwd_login.click()
            time.sleep(3)
            user_num = login_admin()[0]
            password = login_admin()[1]
